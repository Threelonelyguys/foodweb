...
<body>
	...
	<?php echo "<h1>$foodname</h1><br>";
		...
		echo "</ol>";
		echo "<p class='text-success'> Hướng dẫn:</p><br><pre>$createfood</pre><br>";
		echo "<p><span class='text-success'>Tổng số năng lương: </span>$energy KCal</p>";
		$query="UPDATE `custom_dish` SET `customenergy`=$energy WHERE `dishid`=$dishid";
		$result=$database->query($query);
		if ($_SESSION['userid']==1) echo "<a href='./custom_dish_edit.php?i=$dishid'><button class='btn btn-success'>Cập nhật</button></a>";
	?>
	</div>
</body>
</html>